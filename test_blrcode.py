...
class TestRcodes(unittest.TestCase):
    ...
    def setUp(self):
        self.resolver = get_resolver(RESOLVER_LIST)
                
    def test_blacklie_nxdomain(self):
        qname = 'blahblah.documentforce.com'
        qtype = 'A'
        self.assertEqual(rcode(self.resolver, qname, qtype), dns.rcode.NXDOMAIN)

    def test_blacklie_nodata(self):
        qname = 'documentforce.com'
        qtype = 'PTR'
        self.assertEqual(rcode(self.resolver, qname, qtype), dns.rcode.NOERROR)
